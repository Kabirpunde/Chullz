...
interface Props {
  ...
  myUserId: string;
  timerEnds: number;         // unix timestamp when next hand starts
  showdownReady: string[];   // uids who clicked Next Hand
  totalPlayers: number;
...
export default function ShowdownOverlay({
  visible, result, boards, players, myUserId,
  timerEnds, showdownReady, totalPlayers, alreadyVoted, onReadyNextHand, onClose,
}: Props) {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      if (!timerEnds) return;
      const tl = Math.max(0, Math.round(timerEnds - Date.now() / 1000));
      setCountdown(tl);
    };
    tick();
  ...
  const readyCount = showdownReady.length;
  const allReady = readyCount >= totalPlayers && totalPlayers > 0;
  const mins = Math.floor(countdown / 60);
  const secs = countdown % 60;
  const countdownStr = countdown <= 30
    ? `${countdown}s`
  ...
  return (
    <div style={{
      ...
      <div style={{
                    ...
                    return (
                      <div key={uid} style={{
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          <div style={{ fontSize: 11, color: '#ffb800', marginTop: 2 }}>{pts as number} pt{(pts as number) !== 1 ? 's' : ''}</div>
                        </div>
                        <span style={{ fontSize: 14, fontWeight: 900, color: (won as number) > 0 ? '#22c55e' : '#64748b' }}>
                          {(won as number) > 0 ? '+' : ''}{(won as number).toLocaleString()} 🪙
                        </span>
                      </div>
                    );
                  })}
        ...
        {/* Footer: countdown + Next Hand vote */}
        <div style={{
          padding: '12px 16px',
          ...
          <button
            ...
            style={{
              ...
              borderRadius: 14,
              background: alreadyVoted ? '#131a2a' : '#22c55e',
              border: alreadyVoted ? '1px solid #1e293b' : 'none',
              color: alreadyVoted ? '#475569' : '#fff',
              fontSize: 14, fontWeight: 900, cursor: alreadyVoted ? 'default' : 'pointer',
              display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
            }}
          >
            {alreadyVoted ? (
              <><span>✓ Voted</span><span style={{ fontSize: 11, color: '#64748b' }}> — waiting for others ({readyCount}/{totalPlayers})</span></>
            ) : (
              <><span>→ Next Hand</span><span style={{ fontSize: 11 }}>({readyCount}/{totalPlayers} ready)</span></>
            )}
          </button>
          ...
          <div style={{ textAlign: 'center', fontSize: 11, color: '#475569' }}>
            {allReady
              ...
              : `Auto-advancing in ${countdownStr}`
            }
          </div>
        </div>
      </div>
